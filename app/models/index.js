...
const sequelize = new Sequelize(
  ...
  process.env.DB_MYSQL_PASSWORD, // DB 접속 비밀번호
  {
    ...
    port: parseInt(process.env.DB_MYSQL_PORT), // 사용 DB 포트번호
    dialect: process.env.DB_MYSQL_DIALECT, // 현재 사용하는 DB 드라이버
    timezone: process.env.DB_MYSQL_TIMEZONE, // 타임존 (+09:00)
    logging: (process.env.DB_MYSQL_LOG_FLG === 'true' && console.log), // DB Logging
    dialectOptions: {
      dateStrings: true // 문자열로 날짜 받기 위함
    },
    pool: { // 커넥션 풀 설정하기
      ...
      acquire: parseInt(process.env.DB_MYSQL_ACQUIRE_LIMIT), // 연결 최대 대기 시간(ms단위)
      idle: parseInt(process.env.DB_MYSQL_IDLE_LIMIT) // 유휴 커넥션 유지 시간(ms) = 안 쓰는 커넥션 언제까지 유지할 지
    }
  }
);
