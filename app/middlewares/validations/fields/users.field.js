...
// 아이디 필드
export const account = body('account') // body () 안에 유저한테 받은 키를 적음
  .trim()
  .notEmpty()
  .withMessage('아이디는 필수 항목입니다.')
  .bail() // bail 앞에서 틀렸으면 멈추고 이후 처리를 안 함.
  .matches(/^[a-zA-Z0-9]{4,8}$/) 
  .withMessage('영어대/소문자, 숫자, 4~8글자 허용');
...
export const password = body('password')
  ...
  .bail()
  .matches(/^[a-zA-Z0-9!@]{4,8}$/) 
  .withMessage('영어대/소문자, 숫자, 특수문자(!@), 4~8글자 허용');
